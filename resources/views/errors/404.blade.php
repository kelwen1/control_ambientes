...
<body class="bg-gray-50 min-h-screen flex flex-col">
    ...
    <main class="flex-1 max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 w-full flex items-center justify-center">
        <div class="bg-white rounded-xl shadow-lg p-6 sm:p-10 w-full max-w-lg border-l-4 border-[#39B54A]">
            <div class="text-center">
                <div class="mx-auto flex items-center justify-center h-20 w-20 rounded-full bg-gray-100 mb-6">
                    <span class="text-5xl">🔍</span>
                </div>
                <h1 class="text-2xl sm:text-3xl font-bold text-[#39B54A] mb-2">Página no encontrada</h1>
                <p class="text-gray-600 text-sm sm:text-base mb-2">Error 404</p>
                <p class="text-gray-600 text-sm sm:text-base mb-6">La ruta que intentas visitar no existe o ha sido movida.</p>
                <div class="flex flex-col sm:flex-row gap-3 justify-center">
                    @auth
                        <a href="{{ route('dashboard') }}" class="inline-flex justify-center px-6 py-3 bg-[#39B54A] text-white rounded-lg font-semibold hover:bg-[#2d8f3a] transition-colors shadow-lg">
                            Ir al Dashboard
                        </a>
                    @else
                        <a href="{{ route('login') }}" class="inline-flex justify-center px-6 py-3 bg-[#39B54A] text-white rounded-lg font-semibold hover:bg-[#2d8f3a] transition-colors shadow-lg">
                            Iniciar sesión
                        </a>
                    @endauth
                    <a href="{{ url('/') }}" class="inline-flex justify-center px-6 py-3 bg-gray-300 text-gray-700 rounded-lg font-semibold hover:bg-gray-400 transition-colors">
                        Volver al inicio
                    </a>
                </div>
            </div>
