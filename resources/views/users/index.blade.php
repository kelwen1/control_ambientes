...
@section('content')
    ...
    <div class="card-premium bg-white rounded-xl shadow-card overflow-hidden hover:shadow-card-hover transition-shadow duration-300 max-w-full min-w-0">
        <div class="p-4 sm:p-6 md:p-8 border-b border-gray-200 flex flex-col sm:flex-row sm:flex-wrap justify-between items-stretch sm:items-center gap-4 min-w-0">
            ...
            <div class="flex flex-col sm:flex-row sm:flex-wrap gap-2 sm:gap-3 w-full min-w-0 sm:w-auto sm:justify-end sm:items-center">
                ...
                <a href="{{ route('users.role-update') }}"
                   class="btn-primary w-full sm:w-auto shrink-0 px-4 py-2 bg-[#39B54A] text-white rounded-xl hover:bg-[#2d8f3a] hover:shadow-glow transition-all duration-200 shadow-md text-sm sm:text-base font-semibold inline-flex items-center justify-center gap-2">
                    <span>🔄</span>
                ...
                <table class="w-full">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 sm:px-6 py-3 text-left text-xs sm:text-sm font-semibold text-gray-700 uppercase tracking-wider">Creado por</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($users as $user)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-700 font-medium">{{ $user->id_cedula }}</td>
                                <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $user->nombre }} {{ $user->apellido }}</td>
                                <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $user->correo }}</td>
                                <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $user->telefono ?? 'N/A' }}</td>
                                <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $user->user }}</td>
                                <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                    @php
                                        $rolNombre = $user->persona->rol->rol ?? '';
                                    ...
                                    @else
                                        <span class="px-2 py-1 bg-gray-100 text-gray-800 rounded-full text-xs font-medium">{{ $rolNombre ?: 'instructor' }}</span>
                                    @endif
                                </td>
                                <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                    @if($user->creator)
                                        {{ trim($user->creator->nombres . ' ' . $user->creator->apellidos) }}
                                    @else
                                        —
                                    @endif
                                </td>
                                <td class="sticky right-0 bg-white px-4 sm:px-6 py-4 whitespace-nowrap text-sm font-medium z-10">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('users.edit', $user->id_usuario) }}"
                                           class="px-3 py-1.5 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors text-xs sm:text-sm font-medium">✏️ Editar</a>
                                        <button onclick="openDeleteModal('{{ $user->id_usuario }}', '{{ addslashes($user->nombre . ' ' . $user->apellido) }}')"
                                                class="px-3 py-1.5 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors text-xs sm:text-sm font-medium">🗑️ Eliminar</button>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
            ...
            @if($users->hasPages())
                <div class="px-6 sm:px-8 py-4 border-t border-gray-200">
                    <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                        ...
                        <div class="flex items-center gap-2">{{ $users->links() }}</div>
                    </div>
                </div>
            @endif
        @else
            <div class="p-12 text-center">
                ...
                <p class="text-gray-500 mb-6">
                    @if($search) No se encontraron usuarios con "{{ $search }}"
                    @else Comienza agregando un nuevo usuario
                    @endif
                </p>
                @if(!$search)
                    <a href="{{ route('users.create') }}"
                       class="inline-block px-6 py-3 bg-[#39B54A] text-white rounded-lg font-semibold hover:bg-[#2d8f3a] transition-colors shadow-lg transform hover:scale-105">Agregar Primer Usuario</a>
                @endif
            </div>
        @endif
    </div>
        ...
        <div class="modal-container glass-container rounded-2xl shadow-2xl p-6 sm:p-8 w-full max-w-md">
            <div class="text-center">
                <div class="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-red-100 mb-4">
                    <svg class="h-10 w-10 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                </div>
                <h3 class="text-2xl font-bold text-gray-800 mb-2">¿Eliminar Usuario?</h3>
                <p class="text-gray-600 mb-6">¿Estás seguro de que deseas eliminar al usuario <span id="userName" class="font-semibold"></span>? Esta acción no se puede deshacer.</p>
                <div class="flex gap-3 sm:gap-4">
                    <button onclick="closeDeleteModal()" class="flex-1 bg-gray-300 text-gray-700 py-3 rounded-lg font-semibold text-base hover:bg-gray-400 transition-colors shadow-lg">Cancelar</button>
                    <form id="deleteForm" method="POST" class="flex-1" data-base-url="{{ str_replace('/0', '', route('users.destroy', ['id' => 0])) }}">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="w-full bg-red-600 text-white py-3 rounded-lg font-semibold text-base hover:bg-red-700 transition-colors shadow-lg transform hover:scale-105">Eliminar</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <style>
        ...
        .modal-container { animation: modalFadeIn 0.3s ease-out; }
        @keyframes modalFadeIn { from { opacity: 0; transform: scale(0.95) translateY(-10px); } to { opacity: 1; transform: scale(1) translateY(0); } }
        .glass-container { background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(10px); }
        .sticky { position: sticky; }
        tbody tr:hover td.sticky { background-color: #f9fafb; }
        .pagination { display: flex; list-style: none; gap: 0.5rem; margin: 0; padding: 0; }
        .pagination li a, .pagination li span { display: inline-block; padding: 0.5rem 0.75rem; border: 1px solid #d1d5db; border-radius: 0.5rem; text-decoration: none; color: #374151; }
        .pagination li.active span { background-color: #39B54A; color: white; border-color: #39B54A; font-weight: 600; }
        .pagination li.disabled span { color: #9ca3af; cursor: not-allowed; background-color: #f3f4f6; }
    </style>
    <script>
        window.openDeleteModal = function(id, userName) {
            var form = document.getElementById('deleteForm');
            var baseUrl = form && form.dataset && form.dataset.baseUrl ? form.dataset.baseUrl : (window.location.origin + '/s/administracion');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (searchInput && searchForm) searchInput.addEventListener('keypress', function(e) { if (e.key === 'Enter') { e.preventDefault(); searchForm.submit(); } });
        });
    </script>
@endsection
