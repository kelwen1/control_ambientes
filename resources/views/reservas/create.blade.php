        ...
        <form method="POST" action="{{ route('reservas.store') }}" class="space-y-6">
                    ...
                    <select name="id_ambiente" 
                        ...
                        @foreach($ambientes as $ambiente)
                            <option value="{{ $ambiente->id_ambiente }}" 
                                    ...
                                    {{ old('id_ambiente') == $ambiente->id_ambiente ? 'selected' : '' }}>
                    ...
                    <select name="id_ficha" 
                        ...
                        @foreach($fichas as $ficha)
                            <option value="{{ $ficha->id_ficha }}" 
                                    ...
                                    {{ old('id_ficha') == $ficha->id_ficha ? 'selected' : '' }}>
            ...
            <div>
                ...
                <textarea name="observaciones" 
                          ...
                          class="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:border-[#39B54A] focus:outline-none transition-colors text-sm sm:text-base">{{ old('observaciones') }}</textarea>
                @error('observaciones')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
