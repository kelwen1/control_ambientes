...
@section('title', 'Disponibilidad de Ambiente')
...
@section('content')
    <div class="mb-6 sm:mb-8">
        ...
        <p class="text-gray-600 text-sm sm:text-base">Consulta qué días tiene ocupados o libres un ambiente según el día de la semana y la jornada</p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-lg p-4 sm:p-6 md:p-8 mb-6 max-w-full min-w-0">
        ...
        <form method="GET" action="{{ route('ambientes.disponibilidad-ambiente') }}" class="flex flex-col md:flex-row md:flex-wrap items-stretch md:items-end gap-4 w-full min-w-0" id="formDisponibilidad">
            <div class="w-full md:w-auto min-w-0">
                <label for="id_ambiente" class="block text-sm font-semibold text-gray-700 mb-1">Ambiente</label>
                <select name="id_ambiente" id="id_ambiente" required
                    ...
                    @foreach($ambientes as $amb)
                        <option value="{{ $amb->id_ambiente }}" {{ ($id_ambiente ?? '') == $amb->id_ambiente ? 'selected' : '' }}>
                            Ambiente {{ $amb->num_ambiente }}
                        </option>
                    @endforeach
                </select>
            </div>
    ...
    <!-- Leyenda -->
    @if($id_ambiente && $dia_semana && $jornada)
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden max-w-md" style="max-width: min(704px, 100%);">
            <div class="px-5 py-3.5 border-b border-gray-200 flex flex-wrap justify-between items-center gap-2">
                ...
                <div class="flex gap-1">
                    ...
                    @endphp
                    <a href="{{ route('ambientes.disponibilidad-ambiente', array_merge(request()->query(), ['mes' => $mesAnt->month, 'anio' => $mesAnt->year])) }}"
                       class="px-2 py-1 bg-gray-200 hover:bg-gray-300 rounded text-xs font-medium transition-colors">←</a>
                    <a href="{{ route('ambientes.disponibilidad-ambiente', array_merge(request()->query(), ['mes' => now()->month, 'anio' => now()->year])) }}"
                       class="px-2 py-1 bg-gray-200 hover:bg-gray-300 rounded text-xs font-medium transition-colors">Hoy</a>
                    <a href="{{ route('ambientes.disponibilidad-ambiente', array_merge(request()->query(), ['mes' => $mesSig->month, 'anio' => $mesSig->year])) }}"
                       class="px-2 py-1 bg-gray-200 hover:bg-gray-300 rounded text-xs font-medium transition-colors">→</a>
                </div>
            </div>
            <div class="p-4">
    ...
    @else
        <div class="bg-white rounded-xl shadow-lg p-12 text-center text-gray-500">
            <p class="text-lg">Seleccione ambiente, día y jornada, luego pulse «Ver calendario».</p>
        </div>
    @endif
